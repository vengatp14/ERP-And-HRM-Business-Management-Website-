...
$document = find_daily_update_document($documentId);
...
// Only admins, or the employee who owns the update the document
// belongs to, may download it.
$currentUserId = (int) current_user()['id'];
if (!is_admin_role(current_user()) && daily_update_owner((int) $document['update_id']) !== $currentUserId) {
...
audit_log($currentUserId, 'daily_updates', 'document_download', "Downloaded daily update document #{$documentId}: {$document['original_filename']}");
